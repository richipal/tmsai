...
class VannaRemoteClient:
    ...
    def train_ddl(self, ddl):
        ...
        if self.chroma_client and self.ddl_collection:
            ...
            logger.warning("ChromaDB not available, DDL training data not stored")
            return False
    ...
    def train_documentation(self, documentation):
        ...
        try:
            ...
            import re
            table_match = re.search(r"Table\s+(\w+)", table_header, re.IGNORECASE)
            if not table_match:
                logger.error(f"Could not extract table name from: {table_header}")
                return False
            ...
            if self.chroma_client and self.documentation_collection:
                try:
                    ...
                    logger.info(f"Stored documentation for table '{table_name}' in ChromaDB with ID: {doc_id}")
                    return True
                except Exception as e:
                    logger.error(f"Error storing documentation in ChromaDB: {str(e)}")
                    return False
            else:
                logger.warning("ChromaDB not available, documentation not stored")
                return False
        except Exception as e:
    ...
    def train_question_sql(self, question, sql):
        ...
        if self.chroma_client and self.question_sql_collection:
            ...
            logger.warning("ChromaDB not available, question-SQL pair not stored")
            return False
...
class OfficialVannaClient:
    ...
    def remove_training_data(self, id):
        ...
        if not self.vn or not hasattr(self.vn, "remove_training_data"):
            logger.warning("VannaDefault doesn't have remove_training_data method")
            return False
        ...
        try:
            ...
            logger.error(f"Error removing training data with VannaDefault: {str(e)}")
            return False
    ...
    def get_example_questions(self):
        ...
        try:
            ...
            logger.error(f"Error getting example questions with VannaDefault: {str(e)}")
            return [
                "Show me the top 5 products by revenue",
                "How many orders do we have by country?",
                "What is our monthly sales trend for 2023?",
    ...
    def train_documentation(self, documentation):
        ...
        try:
            ...
            logger.error(f"Error training documentation with VannaDefault: {str(e)}")
            return False
    ...
    def train_question_sql(self, question, sql):
        ...
        try:
            ...
            logger.error(f"Error training question-SQL pair with VannaDefault: {str(e)}")
            return False
...
class DirectVannaClient:
    ...
    def __init__(self, api_key=None, model=None):
        ...
        try:
            ...
            logger.warning("OpenAI module not available, direct OpenAI integration will not work")
        except Exception as e:
        ...
        self.knowledge_base = MOCK_TABLES if hasattr(self, 'MOCK_TABLES') else []
    ...
    def generate_sql(self, question):
        ...
        if self.use_openai:
            try:
                ...
                schema_context = "\n".join(self.knowledge_base if self.knowledge_base else MOCK_TABLES)
                ...
                response = self.openai_client.chat.completions.create(
                    model=self.model,
                    messages=[
                        {"role": "system", "content": "You are a database expert that generates SQL from natural language questions."},
    ...
    def ask(self, question):
        ...
        if self.use_openai:
            try:
                ...
                sql_to_explain = ""
                if "what does this SQL do" in question.lower() or "explain this sql" in question.lower():
                    # Try to extract the SQL part from the question
                    import re
                    sql_match = re.search(r"SELECT.*?(?:;|$)", question, re.IGNORECASE | re.DOTALL)
                    if sql_match:
                ...
                response = self.openai_client.chat.completions.create(
                    model=self.model,
                    messages=[
                        {"role": "system", "content": "You are a database expert that explains SQL and database concepts in simple terms."},
                        {"role": "user", "content": prompt}
                    ],
                    temperature=0.3
                )
        ...
        if self.use_openai:
            try:
                ...
                response = self.openai_client.chat.completions.create(
                    model=self.model,
                    messages=[
                        {"role": "system", "content": "You are a helpful assistant that generates example natural language questions for SQL databases."},
                        {"role": "user", "content": prompt}
                    ],
                ...
                try:
                    ...
                    if isinstance(questions, list) and len(questions) > 0:
                        logger.info(f"Generated {len(questions)} example questions with OpenAI")
                        return questions
                except Exception as e:
    ...
    def get_training_data(self):
                ...
                try:
                    ...
                    if collection_data and "documents" in collection_data and "metadatas" in collection_data:
                        ...
                        for i, doc in enumerate(collection_data["documents"]):
                            if i < len(collection_data["metadatas"]) and collection_data["metadatas"][i]:
        ...
        if not result["ddl"]:
            result["ddl"] = self.knowledge_base if self.knowledge_base else MOCK_TABLES
...
def initialize_vanna_client():
    ...
    from .config import (
        API_KEY, VANNA_MODEL, LOCAL_MODE, OPENAI_API_KEY, 
        VANNA_DEFAULT_AVAILABLE, VANNA_CLASS_AVAILABLE
    )
    ...
    if LOCAL_MODE and CHROMADB_AVAILABLE:
        try:
            ...
            return DirectVannaClient(api_key=None, model="local"), "DirectVannaClient"
        except Exception as e:
